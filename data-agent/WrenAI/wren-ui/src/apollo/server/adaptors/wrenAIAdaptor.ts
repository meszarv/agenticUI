...
export class WrenAIAdaptor implements IWrenAIAdaptor {
  ...
  private async waitDeployFinished(deployId: string): Promise<boolean> {
    const deployTimeoutMs = Number(
      process.env.WREN_AI_DEPLOY_TIMEOUT_MS ?? 180000,
    );
    const pollingIntervalMs = Number(
      process.env.WREN_AI_DEPLOY_POLLING_INTERVAL_MS ?? 1000,
    );
    const startedAt = Date.now();

    let deploySuccess = false;
    while (Date.now() - startedAt < deployTimeoutMs) {
      try {
